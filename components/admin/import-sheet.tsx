...
interface Props {
  ...
  open: boolean
  onClose: () => void
  defaultMode?: 'teams-only' | 'participants-only' | 'both'
}

export function ImportSheet({ hackathonId, open, onClose, defaultMode = 'both' }: Props) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const result = await bulkImport({ hackathonId, clearExisting, teams: mergedTeams })
  ...
  return (
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent className="border-white/10 bg-[#222] text-white overflow-y-auto w-full sm:max-w-lg">
        <SheetHeader>
          <SheetTitle className="text-white">Importar Dados</SheetTitle>
        </SheetHeader>
        ...
        <div className="mt-4 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              className="gap-1.5 rounded text-xs font-semibold text-[#636363] hover:bg-white/10 hover:text-white h-8 px-2"
            >
          ...
          <div
            ...
            className="flex cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-white/20 bg-[#2a2a2b] p-8 transition-colors hover:border-[#9810fa]/50 hover:bg-[#9810fa]/[0.03] focus:outline-none focus:ring-2 focus:ring-[#9810fa]"
            onDrop={handleDrop}
            onDragOver={(e) => e.preventDefault()}
            onClick={() => fileRef.current?.click()}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') fileRef.current?.click() }}
          >
            <Upload className="mb-2 h-8 w-8 text-[#636363]" aria-hidden="true" />
            <p className="text-sm font-medium text-[#b2b2b2]">Arraste ou clique para selecionar</p>
            <input
              ref={fileRef}
              type="file"
              accept=".xlsx,.xls,.csv"
          ...
          {importResult && (
            <div role="status" className="flex items-center gap-2 rounded border border-green-500/20 bg-green-500/10 p-3 text-xs text-green-400">
              <CheckCircle className="h-4 w-4 flex-shrink-0" aria-hidden="true" />
              {importResult.teamCount} time{importResult.teamCount !== 1 ? 's' : ''} e{' '}
              {importResult.participantCount} participante{importResult.participantCount !== 1 ? 's' : ''} criados.
            </div>
          )}

          {hasErrors && (
            <div role="alert" className="rounded-xl border border-red-500/20 bg-red-500/[0.06] p-4">
            ...
            <div className="space-y-4">
              ...
              {mode === 'participants-only' && (
                <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#2debb1]">
                  CSV de participantes detectado
                </p>
              )}

              {teams.length > 0 && (
                <div className="rounded-xl border border-white/[0.08] bg-[#2a2a2b] p-4">
                  <h3 className="mb-2 text-[10px] font-semibold uppercase tracking-[0.15em] text-[#636363]">
                    Times ({teams.length})
                  </h3>
                  {teams.map((t, i) => (
                    <div
                      key={`${t.Nome ?? ''}-${i}`}
                      className="flex justify-between border-b border-white/[0.06] py-2 last:border-0 text-sm"
                    >
              ...
              <label className="flex cursor-pointer select-none items-center gap-3">
                ...
                />
                <span className="text-sm text-[#b2b2b2]">Substituir dados existentes</span>
              </label>
